package dev.yuua.journeylib.discord.framework.function.command

import dev.minn.jda.ktx.interactions.updateCommands
import dev.yuua.journeylib.discord.framework.Framework
import dev.yuua.journeylib.discord.framework.function.FrFunctionManagerStruct
...
import dev.yuua.journeylib.discord.framework.function.command.router.FrCmdRouteDB
...
class FrCommandManager(override val framework:Framework) : FrFunctionManagerStruct<FrCommandStruct> {

    private val libFlow: LibFlow = LibFlow(this.javaClass.simpleName)

    override fun record(script: FrFunctionManagerStruct<FrCommandStruct>.() -> Unit) {
        ...
        for (instance in instances) {
            val frCommand = instance.cmd().cmd
            val jdaCommandData: SlashCommandData = instance.cmd().cmd.jdaCmdData

            // Target Guilds (Empty if scope is not defined)
